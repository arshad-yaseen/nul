...
/// Bindings only, a capture say. Nothing to emit on the way out.
...
fn checkArgument(
    ...
    // `&place` lives only here, addressing the place directly
...
fn emitBuiltin(
    ...
    switch (call.bound) {
        ...
        .arena_copy => {
            ...
            const result = try check.emit(.arena_copy, return_type, .{
